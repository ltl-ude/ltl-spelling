...
public class EvaluateErrorDetection extends JCasAnnotator_ImplBase {
	...
	public void process(JCas aJCas) throws AnalysisEngineProcessException {
		...
		for(Token token : JCasUtil.select(aJCas, Token.class)) {
			...
			if(error_gold == 1 && error_predicted == 0) {
				FN.inc(token.getCoveredText()+"\t("+JCasUtil.selectCovered(SpellingError.class, token).get(0).getCorrection()+")");
			}
			if(error_gold == 0 && error_predicted == 1) {
				if(!JCasUtil.selectCovered(GrammarError.class, token).isEmpty()) {
					...
					FP.inc("_grammar_error: "+token.getCoveredText()+"\t("+JCasUtil.selectCovered(GrammarError.class, token).get(0).getCorrection()+")");
				}
				else {
	...
	public void collectionProcessComplete() throws AnalysisEngineProcessException {
		...
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter(new File(SpellingConstants.EVALUATION_DATA_PATH+configName+"/prec_rec_f1.txt")));
			
			System.out.println(confusion_matrix[0][0]+" ("+TN.getKeys().size()+" distinct)\t"+confusion_matrix[1][0]+" ("+FN.getKeys().size()+" distinct)");
			System.out.println(confusion_matrix[0][1]+" ("+FP.getKeys().size()+" distinct)\t"+confusion_matrix[1][1]+" ("+TP.getKeys().size()+" distinct)");
			System.out.println(numberOfFPthatAreGrammarErrors+" of the false positives are grammar errors.");
			
			bw.write(confusion_matrix[0][0]+" ("+TN.getKeys().size()+" distinct)\t"+confusion_matrix[1][0]+" ("+FN.getKeys().size()+" distinct)");
			bw.newLine();
			bw.write(confusion_matrix[0][1]+" ("+FP.getKeys().size()+" distinct)\t"+confusion_matrix[1][1]+" ("+TP.getKeys().size()+" distinct)");
			bw.newLine();
			bw.write(numberOfFPthatAreGrammarErrors+" of the false positives are grammar errors.");
			bw.newLine();
			
			double precision = confusion_matrix[1][1]*1.0 / (confusion_matrix[1][1]*1.0 + confusion_matrix[0][1]*1.0);
			double recall = confusion_matrix[1][1]*1.0 / (confusion_matrix[1][1]*1.0 + confusion_matrix[1][0]*1.0);
			double f_1 = (2 * precision * recall)/(precision + recall);
			
			System.out.println("Precision:\t"+precision);
			System.out.println("Recall:\t"+recall);
			System.out.println("F-Score:\t"+f_1);
			
			bw.write("Precision:\t"+precision);
			bw.newLine();
			bw.write("Recall:\t"+recall);
			bw.newLine();
			bw.write("F-Score:\t"+f_1);
			bw.newLine();
			bw.close();	
			
			bw = new BufferedWriter(new FileWriter(new File(SpellingConstants.EVALUATION_DATA_PATH+configName+"/false_positives.txt")));
			...
			bw = new BufferedWriter(new FileWriter(new File(SpellingConstants.EVALUATION_DATA_PATH+configName+"/false_negatives.txt")));
			...
			bw = new BufferedWriter(new FileWriter(new File(SpellingConstants.EVALUATION_DATA_PATH+configName+"/true_negatives.txt")));
			...
			bw = new BufferedWriter(new FileWriter(new File(SpellingConstants.EVALUATION_DATA_PATH+configName+"/true_positives.txt")));
